...
class CurrencyExchange 
  ...
  def self.currency_exchange(amount, currency, with_base_currency=nil)
    with_base_currency ||= @@base_currency || DEFAULT_BASE_CURRENCY
    
    raise(NOT_POSSIBLE+" #{currency[0..2]} AND #{with_base_currency[0..2]}") if 
      (@currency=currency_rate_exists?(currency)).nil? || 
      (@base_currency=currency_rate_exists?(with_base_currency)).nil? 
    
    if currency == with_base_currency
      amount
    else
  ...
  def self.convert(from_currency,to_currency,amount)
    (((amount.to_f * 10**2) / from_currency.rate).round.to_f * to_currency.rate) / 10**2
  end
